...
import './App.css'
import iccCourse from './assets/icc_course.json'
...
const MODULES = (iccCourse && Array.isArray(iccCourse.modules) && iccCourse.modules.length > 0)
  ? iccCourse.modules
  : buildModules()

// Ensure each module has a valid quizBank; prefer embedded `quizBank` from
// `icc_course.json` (generated by scripts/generate_quizzes.cjs). Fallback to
// the original QUIZ_TEMPLATES mapped per module when missing.
MODULES.forEach((module, moduleIndex) => {
  if (!Array.isArray(module.quizBank) || module.quizBank.length === 0) {
    module.quizBank = QUIZ_TEMPLATES.map((question, questionIndex) => ({
      ...question,
      id: `${module.id || `module-${moduleIndex + 1}`}-Q${questionIndex + 1}`,
      prompt: `[${module.topic || 'Module'}] ${question.prompt}`,
    }))
  }
})
